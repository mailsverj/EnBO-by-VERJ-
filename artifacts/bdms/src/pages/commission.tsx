...
import { Search, Filter, Wallet, ArrowUpRight, CheckCircle2, Loader2 } from 'lucide-react';
...
export default function Commission() {
  ...
  const totalLiability = commissions.reduce((sum, c) => sum + c.amount, 0);
                    ...
                    filteredComms.map(record => (
                      <TableRow key={record.id} className="hover:bg-muted/50 transition-colors">
                        ...
                        <TableCell>
                          ...
                          <Link href={`/invoicing/${record.invoiceRef}`} className="text-xs text-muted-foreground hover:text-primary hover:underline">{record.invoiceRef}</Link>
                        </TableCell>
                        ...
                        <TableCell className="text-sm">{format(new Date(record.createdAt), 'MMM d, yyyy')}</TableCell>
                        <TableCell>
