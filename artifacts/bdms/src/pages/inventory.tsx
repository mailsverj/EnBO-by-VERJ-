...
import { Search, Filter, Plus, Edit2, History, Loader2 } from 'lucide-react';
import { useAuth } from '@/store/auth';
import { Dialog, DialogContent, DialogHeader, DialogTitle } from '@/components/ui/dialog';
import { format } from 'date-fns';

function PriceHistoryDialog({ sku, open, onClose }: { sku: string | null; open: boolean; onClose: () => void }) {
  const { data, isLoading } = useQuery({
    queryKey: ['inventory-item', sku],
    queryFn: () => api.inventory.get(sku!),
    enabled: !!sku,
  });
            ...
            <TableBody>
              {auditHistory.map(audit => (
                <TableRow key={audit.id}>
                  ...
                  <TableCell className="text-right text-sm">{audit.changedByName}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
...
export default function Inventory() {
  ...
  const serverCanSeePrices = data?.canSeePrices ?? canSeePrices();
    ...
    <div className="space-y-6 animate-in fade-in duration-500 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage equipment catalog{serverCanSeePrices && ' and pricing'}.</p>
        </div>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredItems.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={serverCanSeePrices ? 8 : 6} className="h-24 text-center text-muted-foreground">
                    ...
                    filteredItems.map(item => (
                      <TableRow key={item.sku} className="hover:bg-muted/50 transition-colors">
                        ...
                        {serverCanSeePrices && (
                          <>
                            <TableCell className="text-right text-muted-foreground">{item.costPrice != null ? formatCurrency(item.costPrice) : '-'}</TableCell>
                            <TableCell className="text-right font-medium text-primary">{item.sellingPrice != null ? formatCurrency(item.sellingPrice) : '-'}</TableCell>
                          </>
                        )}
      ...
      <PriceHistoryDialog sku={auditSku} open={!!auditSku} onClose={() => setAuditSku(null)} />
    </div>
  );
}
